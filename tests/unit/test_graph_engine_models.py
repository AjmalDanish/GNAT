...
class TestDatasetModel:
    ...
    def test_dataset_status_methods(self, db):
        """Test dataset status methods."""
        dataset = Dataset.objects.create(
            dataset_name="Test Dataset",
            version="1.0.0",
            status=Dataset.DatasetStatus.PENDING,
        )
        dataset.mark_generating()
        dataset.refresh_from_db()
        assert dataset.status == Dataset.DatasetStatus.GENERATING

        dataset.mark_completed(1000)
        dataset.refresh_from_db()
        assert dataset.status == Dataset.DatasetStatus.COMPLETED
        assert dataset.record_count == 1000

        dataset.mark_failed()
        dataset.refresh_from_db()
        assert dataset.status == Dataset.DatasetStatus.FAILED
